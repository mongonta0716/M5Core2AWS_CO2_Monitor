...
static int mhz19_cmd(mhz19_config_t* config, mhz19_cmd_t cmd, uint32_t val) {
	...
	for (int i=0;i<9;i++) {
		ESP_LOGI(TAG, "packet:%d:%d", i,packet[i]);
	}
	int result = mhz19_check(packet);
	ESP_LOGI(TAG, "CheckResult:%d", result);
		...
		if(len == 9){
			...
			if(len == 9 && mhz19_check(data)){
				if(data[0]==0xff && data[1]==0x86) {
					...
					ESP_LOGI(TAG, "CO2: %d T:%d S:%d U:%d",co2val, t, s, u);
					if (co2val > 400 && co2val <= 5000) {
						public_result.mhz19.temp = t;
						public_result.mhz19.co2  = co2val;
					}
				}
			}
		}
...
static void mhz19_task(mhz19_config_t* config) {
	...
	if (config->interval > 0) {
		vTaskDelay(config->interval/portTICK_PERIOD_MS);
	} else {
		break;
	}
    }
...
int mhz19_getCO2() {
	return public_result.mhz19.co2;	
}
...
esp_err_t mhz19_init(mhz19_config_t* config) {
	...
	xTaskCreate((TaskFunction_t)mhz19_task, task_name, 1024*3, config, 8, NULL);
	return ESP_OK;	//todo
}
#define SMA_SIZE 60
