...
// Parameters for CO2 Monitor
static bool door_open = false;
#define ROOM_CLEAN    0
#define ROOM_WARNING  1
#define ROOM_DANGER   2
uint8_t room_condition = ROOM_CLEAN;
#define CO2_TOO_HIGH 1500
#define CO2_LOW 1000
...
void door_Callback(const char *pJsonString, uint32_t JsonStringDataLen, jsonStruct_t *pContext) {
    IOT_UNUSED(pJsonString);
    IOT_UNUSED(JsonStringDataLen);
    ...
    if(pContext != NULL) {
        ESP_LOGI(TAG, "Door state changed to %s", status);
    }

    if(strcmp(status, DOOR_OPEN) == 0) {
        ESP_LOGI(TAG, "setting side LEDs to red");
        Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_LEFT, 0x00FF00);
        Core2ForAWS_Sk6812_Show();
        ui_door_label_update("Door Open ");
        door_open = true;
    } else if(strcmp(status, DOOR_CLOSE) == 0) {
        ESP_LOGI(TAG, "setting side LEDs to blue");
        Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_LEFT, 0x0000FF);
        Core2ForAWS_Sk6812_Show();
        ui_door_label_update("Door Close");
        door_open = false;
    } else {
        ESP_LOGI(TAG, "clearing side LEDs");
...
char hvacStatus[8] = STARTING_HVACSTATUS;
char doorStatus[3] = STARTING_DOORSTATUS; 
bool roomOccupancy = STARTING_ROOMOCCUPANCY;
int co2 = STARTING_CO2;
...
void aws_iot_task(void *param) {
    ...
    jsonStruct_t doorStatusActuator;
    doorStatusActuator.cb = door_Callback;
    doorStatusActuator.pKey = "doorStatus";
    doorStatusActuator.pData = &doorStatus;
    doorStatusActuator.type = SHADOW_JSON_STRING;
    doorStatusActuator.dataLength = strlen(doorStatus)+1;
    ...
    co2Handler.type = SHADOW_JSON_UINT32;
    co2Handler.dataLength = sizeof(uint32_t);
...
//    xTaskCreatePinnedToCore(&microphone_task, "microphone_task", 4096, NULL, 1, NULL, 1);
    ...
    rc = aws_iot_shadow_register_delta(&iotCoreClient, &doorStatusActuator);
    if(SUCCESS != rc) {
        ESP_LOGE(TAG, "Shadow Register Delta Error(door)");
    }
    ...
    while(NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc) {
        ...
        if (co2 > CO2_TOO_HIGH)  {
            if (door_open == false) {
                ESP_LOGI(TAG, "RoomCondition:DANGER");
                ui_textarea_add("CO2 levels on the rize. Please Open the door to ventilate!\n", NULL, 0);
                Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_RIGHT, 0xFF0000);
                Core2ForAWS_Sk6812_Show();
                room_condition = ROOM_DANGER;
                Core2ForAWS_Speaker_Enable(1);
                ESP_ERROR_CHECK(Speaker_Init());
                Speaker_WriteBuff((uint8_t *)co2_high_raw, co2_high_raw_len, 1000);
                Speaker_Deinit();
            } else {
                ESP_LOGI(TAG, "RoomCondition:WARNING");
                room_condition = ROOM_WARNING;
                ui_textarea_add("CO2 level is too high. Ventilating.\n", NULL, 0);
                Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_RIGHT, 0xFFFF00);
                Core2ForAWS_Sk6812_Show();
            }
        }
        if ((co2 < CO2_LOW) && ((room_condition == ROOM_WARNING) || (room_condition == ROOM_DANGER))) {
            ESP_LOGI(TAG, "RoomCondition:CLEAN");
            ui_textarea_add("The room is well ventilated. You can close the door.\n", NULL, 0);
            room_condition = ROOM_CLEAN;
            Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_RIGHT, 0x00FF00);
            Core2ForAWS_Sk6812_Show();
            Core2ForAWS_Speaker_Enable(1);
            ESP_ERROR_CHECK(Speaker_Init());
            Speaker_WriteBuff((uint8_t *)co2_low_raw, co2_low_raw_len, 1000);
            Speaker_Deinit();
        }

        // END get sensor readings
        uint8_t mqtt_switch = ui_mqtt_get_value();
        if (mqtt_switch == 1) {
            ...
            ESP_LOGI(TAG, "On Device: doorStatus %s", doorStatus);
            ...
            if(SUCCESS == rc) {
                rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 5, &temperatureHandler,
                                                &soundHandler, &roomOccupancyActuator, &hvacStatusActuator, &doorStatusActuator, &co2Handler);
                if(SUCCESS == rc) {
...
{   
    ...
    Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_RIGHT, 0x00FF00); // room clean
    Core2ForAWS_Sk6812_SetSideColor(SK6812_SIDE_LEFT , 0x0000FF); // door closed
    Core2ForAWS_Sk6812_Show();
    ...
    Core2ForAWS_Speaker_Enable(1);
    ESP_ERROR_CHECK(Speaker_Init());
    Speaker_WriteBuff((uint8_t *)co2_start_raw, co2_start_raw_len, 1000);
    Speaker_Deinit();
    initialise_wifi();
    start_mhz19();
